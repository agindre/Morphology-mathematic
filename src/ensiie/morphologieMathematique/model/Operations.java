...
public class Operations {
    ...
    /**
     * Fonction test_appart : Implémentation différente de la fonction d'appartenance mu, qui, lors de l'érosion et de la dilatation a pour effet de faire ressortir les zones à fort différentiel de niveau de gris
     * Non-implémenté, car ne correspond pas au besoin de l'application
     * @param x le niveau de gris sur l'intervalle [0,1] du point de l'élément structurant courant
     * @return le niveau de gris sur l'intervalle [0,1] du point
     */
    private static double test_appart(double x) {
        double res = (double) (Math.sin(Math.PI * x) + (Math.sin(2 * Math.PI * x) / 2) + (Math.sin(3 * Math.PI * x) / 3) - 0.4);
        return res;
    }
